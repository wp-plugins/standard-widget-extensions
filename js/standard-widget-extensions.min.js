(function(c,g,x,z){c(x).ready(function(){function k(a,b,y,d,e){a.o=null;a.height=0;a.fixed=0;a.default_offset={top:0,left:0};a.margin_top=0;a.margin_left=0;a.width=0;a.absolute_adjustment_top=0;a.absolute_adjustment_left=0;a.percent_width=0;a.disable_iflt=0;a.float_attr_check_mode=parseInt(e,10);a.mode=0;a.previoustop=0;b&&(a.id="#"+b,a.id&&0<c(a.id).length&&(a.o=c(a.id),a.parent=a.o.parent(),a.margin_top=parseInt(a.o.css("margin-top"),10),a.percent_width=parseFloat(y),a.disable_iflt=parseInt(d,10)))}
...
b.window_height?2:1};swe.resizeHandler=h;c(g).scroll(s);c(g).resize(h);if(swe.enable_reload_me){var w=c(g).width();c("body").append('<div class="hm-swe-modal-background"><div class="hm-swe-resize-message"><p>'+swe.msg_reload_me+'</p><input type="button" id="hm-swe-reload-button" value="'+swe.msg_reload+'" style="margin: 10px 20px" /><input type="button" id="hm-swe-continue-button" value="'+swe.msg_continue+'" style="margin: 10px 20px" /></div></div>');c(g).resize(function(){c(g).width()!=w&&(c(".hm-swe-modal-background").css("display",
"block"),w=c(g).width())});c(".hm-swe-modal-background").click(function(){c(".hm-swe-modal-background").css("display","none")});c("#hm-swe-reload-button").click(function(){location.reload()})}swe.recalc_after=parseInt(swe.recalc_after,10);swe.recalc_count=parseInt(swe.recalc_count,10);h(!0)}swe.reloadHandler=function(){swe.accordion_widget&&q();swe.scroll_stop&&c(m).length&&(k(d,swe.sidebar_id,swe.proportional_sidebar,swe.disable_iflt,swe.float_attr_check_mode),k(e,swe.sidebar_id2,swe.proportional_sidebar2,
swe.disable_iflt2,swe.float_attr_check_mode2),swe.resizeHandler())}})})(jQuery,window,document);
